...
ulDir* ulOpenDir ( const char* dirname )
{
  ...
  if ( dir != NULL )
  {
    ...
    dir->done = false;
#ifdef UNDER_CE
	wchar_t search_w[UL_NAME_MAX];
	mbstowcs(search_w, search, strlen(search)+1);
    dir->hFind = FindFirstFile(search_w, &dir->data);
#else
...
ulDirEnt* ulReadDir ( ulDir* dir )
{
  ...
  if ( dir->done )
    return NULL ;
#ifdef UNDER_CE
	char file_name[UL_NAME_MAX];
	wcstombs(file_name, dir->data.cFileName, wcslen(dir->data.cFileName)+1);
	strcpy( dir->curr.d_name, file_name ) ;
