...
long USNG_Get_Latitude_Band_Min_Northing(long letter, double* min_northing)
/*
...
long Round_USNG (double value)
/*
 * The function Round_USNG rounds the input value to the
...
{ /* BEGIN UTM_To_USNG */
  ...
  divisor = pow (10.0, (5 - Precision));
  Easting = Round_USNG (Easting/divisor) * divisor;
  Northing = Round_USNG (Northing/divisor) * divisor;
  ...
  USNG_Get_Grid_Values(Zone, &ltr2_low_value, &ltr2_high_value, &false_northing);
...
{ /* Convert_Geodetic_To_USNG */
  ...
  if (!error_code)
  {
    ...
    {
      Set_UPS_Parameters (USNG_a, USNG_f);
...
{ /* Convert_UTM_To_USNG */
  ...
  if (!error_code)
  {
    ...
    { /* Reconvert to UTM zone 32 */
      Set_UTM_Parameters (USNG_a, USNG_f, 32);
    ...
    if (!error_code)
    {
      ...
      else
      {
        ...
        if (!error_code)
        {
          ...
          error_code = USNG_Get_Latitude_Band_Min_Northing(letters[0], &min_northing);
          if (!error_code)
          {
            ...
            *Northing = grid_northing + *Northing;
            ...
            error_code = Set_UTM_Parameters(USNG_a,USNG_f,*Zone);
            if (!error_code)
            {
              error_code = Convert_UTM_To_Geodetic(*Zone,*Hemisphere,*Easting,*Northing,&latitude,&longitude);
              if (!error_code)
